...
def confirm_payment():
    ...
    if event.type == 'payment_intent.succeeded':
        ...
        trello_due = datetime.today() + timedelta(3)
        trello_helper.create_customer_card(trello_helper.PAYMENT_RECIEVED_LIST_ID, trello_title, desc=trello_description, due=str(trello_due), labels=[trello_helper.ONLINE_ORDER_LABEL], position='top')
    else:
        abort(400)
    return '', 204
